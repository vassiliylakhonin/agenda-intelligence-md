...
def test_clean_escrow_release():
    ...
    assert ruling.is_released_to_seller is True
    assert ruling.is_refunded_to_buyer is False
    assert ruling.ruling == "RELEASE_TO_SELLER"
    assert ruling.status == "decision_ready"
    assert ruling.score == 95
    assert ruling.checks["deadline_honored"] is True
    assert ruling.checks["hash_verified"] is True
    assert ruling.checks["slo_verified"] is True
    assert len(ruling.violations) == 0

    assert isinstance(ruling.payout, PayoutBreakdown)
    assert ruling.payout.total_escrow_usd == 500.0
    assert ruling.payout.seller_payout_usd == 495.0
    assert ruling.payout.buyer_refund_usd == 0.0
    assert ruling.payout.arbiter_fee_usd == 5.0


def test_hash_mismatch_refund():
    arbiter = M2MEscrowArbiter()
    deal_terms = {
        "buyer_id": "did:agent:0x1111",
        "seller_id": "did:agent:0x2222",
    ...
    assert ruling.is_refunded_to_buyer is True
    assert ruling.ruling == "REFUND_TO_BUYER"
    assert ruling.checks["hash_verified"] is False
    assert ruling.payout.seller_payout_usd == 0.0
    assert ruling.payout.buyer_refund_usd == 495.0
    assert ruling.payout.arbiter_fee_usd == 5.0
...
def test_pro_rata_partial_settlement():
    ...
    assert ruling.is_partial_settlement is True
    assert ruling.score == 80
    assert ruling.checks["slo_verified"] is False
    assert ruling.payout.seller_payout_usd == 792.0
    assert ruling.payout.buyer_refund_usd == 198.0
    assert ruling.payout.arbiter_fee_usd == 10.0
...
def test_all_or_nothing_slo_deficit_refund():
    ...
    assert ruling.is_refunded_to_buyer is True
    assert ruling.payout.seller_payout_usd == 0.0
    assert ruling.payout.buyer_refund_usd == 990.0
    assert ruling.payout.arbiter_fee_usd == 10.0
