...
- One model, one prompt run, one author scoring. Not statistically significant. The methodology says so up front.
- Structural delta only. A high-delta output that is factually wrong is still wrong. Several B-side facts (specific dates, statute names) would need primary-source verification before any operational use; this eval did not perform that verification.
- The scoring is binary-leaning and intentionally narrow. A pure prose memo that conveys the same epistemic discipline without tables could legitimately score lower than B did and still be useful.
- Self-scored by the case author. The methodology explicitly accepts this for v0.9.
- This case isolates GTTA from regional and sector specialists by design. It does not validate routing accuracy or vertical-lens lift; the stub cases [`ca-caspian-sanctions.md`](ca-caspian-sanctions.md) and [`gulf-me-hormuz-shipping.md`](gulf-me-hormuz-shipping.md) exist for those surfaces and remain stubs as of this run.
- The two field corrections (`scenario_brief` -> `scenario`, audience -> `founder`) were schema-validation lift, but they also represent silent caller intent loss. Treat both as features-with-caveats rather than pure wins.
