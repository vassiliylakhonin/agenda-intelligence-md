...
# Optional: diagnose the local install and MCP tool surface
...
- **MCP stdio server** – `agenda-intelligence-mcp` exposes `validate_brief`, `validate_evidence`, `get_protocol`, `list_lenses`, `get_lens`, `source_plan`, `score_output`.
- **Self-diagnosis** – `doctor` checks local package assets, MCP config output, and MCP `tools/list`.
