...
Product runtime and evidence-discipline layer for strategic intelligence agents. One core service layer behind four delivery surfaces — MCP server, HTTP API, A2A adapter, and a deployable Cloudflare Worker baseline — plus structured per-product contracts, geography-routed reasoning, schema validation, evidence audit, and scoring. Ships with two vertical workers: Middle Corridor Deal Risk Gate and CIS Secondary-Sanctions Exposure. Per-profile live retrieval is declared as a capability for `cis_secondary_sanctions` against OpenSanctions ([ADR 0014](docs/adr/0014-per-profile-live-retrieval.md)) but **runtime activation is currently deferred** because the hosted OpenSanctions API is paid (€0.10/call) and no buyer has been confirmed; the profile operates on user-supplied evidence only until `OPENSANCTIONS_API_KEY` is configured. No factual-truth verification.
...
This profile **declares the capability** for per-profile live retrieval against the [OpenSanctions](https://www.opensanctions.org) consolidated dataset (CC-BY 4.0 for the data; the hosted API itself is paid €0.10/call) per [ADR 0014](docs/adr/0014-per-profile-live-retrieval.md). **Runtime activation is currently deferred** per the 2026-05-27 update to ADR 0014: the hosted API is paid and no buyer has been confirmed for the profile, so the project has not committed to per-call spending. To re-activate locally or in production, set `OPENSANCTIONS_API_KEY` (30-day business-email trial at https://www.opensanctions.org/api/, then €0.10/call). Without the key the service degrades gracefully and triage is based on user-supplied evidence only — `live_retrieval_status: disabled` in the response and `boundaries.live_retrieval: false` in `/status`.
