...
**Contract-only pack (not a vertical worker yet):**

- **Kazakhstan market-entry readiness gate** — a deliberately contract-first pack: request/response schemas ([schemas/v1/market-entry-readiness-request.schema.json](schemas/v1/market-entry-readiness-request.schema.json) + [response](schemas/v1/market-entry-readiness-response.schema.json)), source taxonomy ([source-requirements/kazakhstan-market-entry-readiness.json](source-requirements/kazakhstan-market-entry-readiness.json)), use-case doc ([docs/use-cases/kazakhstan-market-entry-readiness.md](docs/use-cases/kazakhstan-market-entry-readiness.md)), contract examples ([examples/kazakhstan-market-entry-readiness/contract/](examples/kazakhstan-market-entry-readiness/contract/)), and a static contract test ([tests/test_market_entry_readiness_contract.py](tests/test_market_entry_readiness_contract.py)). It has **no service function, HTTP route, A2A profile, or MCP tool** — the "MUST ship with" checklist below applies the day it gains a service function, not before. Until then it is a contract, not a worker: do not describe it as live or deployed anywhere, and do not register it on any transport surface without the full checklist.

**Rule for adding the next vertical worker:**

- New workers live inside this repo by default: same service layer, same schemas/v1/ directory, same A2A adapter, one published Cloudflare Worker per product. The third-worker spin-off decision was made and recorded in [ADR 0016](docs/adr/0016-keep-vertical-workers-in-one-repo.md): **keep all workers together** — none of the spin-off triggers (divergent release cadence, divergent dependency footprint, separate buyer / channel, separate-license commercial reason) fire at current scale.
