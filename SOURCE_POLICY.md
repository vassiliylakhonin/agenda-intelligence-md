...
|---|---|---|---|---|
| Snapshot (compact public-list name index, fetched from a verified public URL) | Public official lists (OFAC SDN + consolidated, EU, UK FCDO) | `cis_secondary_sanctions` (preferred, $0) | **Deferred** — the proposed URL was removed because the index was never published; activate only after publishing and verifying `SNAPSHOT_INDEX_URL` (ADR 0020 update) | Counterparty name → exact + token-overlap matches against a fresh public-list snapshot, matched in-worker (no external host) |
