...
def _memo_quality_failed_guardrails(result: dict) -> set[str]:
    failed: set[str] = set()
...
def _load_memo_quality_manifest(root: Path) -> dict | None:
    ...
    try:
        ...
        raise SystemExit(f"Invalid memo-quality manifest JSON: {e}")
    if not isinstance(manifest, dict):
        raise SystemExit("Memo-quality manifest must be a JSON object")
    fixtures = manifest.get("fixtures")
    if not isinstance(fixtures, list):
        raise SystemExit("Memo-quality manifest must contain fixtures[]")
    return manifest


def _memo_quality_manifest_errors(root: Path, manifest: dict, cases: list[dict]) -> list[str]:
...
def _memo_quality_bench_cases(root: Path) -> tuple[list[dict], dict | None, list[str]]:
    from agenda_intelligence.mcp_server import check_memo_quality

    manifest = _load_memo_quality_manifest(root)
    cases: list[dict] = []
    for expected_class in ("golden", "failure"):
        ...
        for path in sorted(directory.glob("*.json")):
            ...
            failed_guardrails = sorted(_memo_quality_failed_guardrails(result))
    ...
    manifest_errors = _memo_quality_manifest_errors(root, manifest, cases) if manifest else []
    return cases, manifest, manifest_errors


def _summarize_memo_quality_bench(cases: list[dict], manifest: dict | None, manifest_errors: list[str]) -> dict:
...
def cmd_memo_quality_bench(args):
    ...
    cases, manifest, manifest_errors = _memo_quality_bench_cases(root)
    if not cases:
        raise SystemExit(f"No memo-quality fixtures found under {root}/golden or {root}/failure")
    summary = _summarize_memo_quality_bench(cases, manifest, manifest_errors)
