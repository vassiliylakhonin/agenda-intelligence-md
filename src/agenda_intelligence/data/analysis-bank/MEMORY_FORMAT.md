...
- Keep `memory_index.json` synchronized with every card in `successes/` and
  `failures/`; `agenda-intelligence memory-lint` is the contract check.
- `memory-search` returns only active, unexpired lessons unless
  `--include-inactive` is passed for audit work.
