// Vizier Corridor & Sanctions Risk Assistant Counterparty Sanctions (OFAC 50% Rule) & Query DLP Firewall Adapter.
//
// Connects Worker #10 (corridor-sanctions-assistant-a2a, profile: corridor_sanctions_assistant)
// to the Vizier security kernel, enabling real-time screening of mentioned counterparties
// under the OFAC 50% Rule, as well as prompt / query DLP firewall inspection across free-form
// caller inquiries and structured parameters to prevent credential and sensitive data leaks.
//
// Dual transport:
...
export const ASSISTANT_NOTICE =
  "Corridor & sanctions orientation counterparty screening (OFAC 50% Rule) & query DLP firewall via Vizier Action Firewall (https://vizier.vassiliy-lakhonin.workers.dev).";

export function assistantAttributionBlock() {
  return {
    upstream: "Vizier Corridor Sanctions Assistant Firewall",
    url: VIZIER_DEFAULT_URL,
    notice: ASSISTANT_NOTICE
  };
}

export function isAssistantVizierEnabled(env = {}) {
...
/**
 * Extracts candidate counterparty / entity names from caller query text and parameters.
 *
 ...
 */
export function extractAssistantEntities(text = "", params = {}) {
  ...
  if (typeof text === "string" && text.length > 0) {
    ...
    // 3. Scan text for explicit role mentions like "counterparty is X", "entity: X", "partner X", "vessel X"
    const explicitRegex = /(?:counterparty|entity|company|vessel|partner|operator|forwarder|trader|carrier|firm)(?:\s+is|\s*:|\s+called|\s+named)?\s+([A-Z][a-zA-Z0-9_\-&]+(?:\s+[A-Z][a-zA-Z0-9_\-&]+)*)/gi;
    let match;
    ...
    // 4. Scan text for carrier / operator / handler mentions
  ...
  // Cap at 5 entities to ensure snappy edge response
  return entitiesToScreen.slice(0, 5);
}

/**
 * Verifies a Corridor Sanctions Assistant inquiry against Vizier for sanctions (OFAC 50% Rule) and DLP leaks.
 *
 * @param {object} env - Worker environment bindings
 * @param {string} text - Free-form caller text
 * @param {object} [params] - Request parameters
 * @param {object} [options]
 * @returns {Promise<object>}
 */
export async function verifyAssistantWithVizier(env = {}, text = "", params = {}, options = {}) {
  if (!isAssistantVizierEnabled(env)) {
    return {
      ...
      receipt: null,
      attribution: assistantAttributionBlock(),
      queried_at: nowIso(),
  ...
  const headers = {
    ...
    "User-Agent":
      "agenda-intelligence-md/corridor_sanctions_assistant (+https://github.com/vassiliylakhonin/agenda-intelligence-md)"
  };
  ...
  const entitiesToScreen = extractAssistantEntities(text, params);
