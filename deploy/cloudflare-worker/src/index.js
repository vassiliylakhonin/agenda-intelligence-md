  ...
  return {
    ...
    gates: [
      ...
      },
      {
        ...
        description: "Escrow evidence evaluator: checks supported schemas and hashes, proposes allocations, and escalates unverifiable evidence.",
...
async function a2aResultForAgentFinancialGuard(params, request, env = {}) {
  ...
  const artifactText = [
    ...
    verdict.vizier_clearance_receipt ? `\nVizier receipt: \`${verdict.vizier_clearance_receipt}\`` : ""
  ]
    .filter(Boolean)
    .join("\n");
...
async function a2aResultForM2MEscrowArbiter(params, request, env = {}) {
  ...
  const artifactText = [
    ...
    ruling.vizier_clearance_receipt ? `\nVizier receipt: \`${ruling.vizier_clearance_receipt}\`` : ""
  ]
    .filter(Boolean)
    .join("\n");
