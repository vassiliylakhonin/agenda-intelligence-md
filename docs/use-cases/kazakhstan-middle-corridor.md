...
When vessel or carrier history is not among the supplied sources, the response carries `vessel_due_diligence_indicators`: a deceptive-shipping-practice (DSP) verification checklist (AIS continuity, vessel-identity consistency, certificate-of-origin integrity, ship-to-ship transfer history, flag history) drawn from OFAC maritime guidance. Same pattern and boundary as the other checklists — an evidence-gap checklist routed to human review, not vessel adjudication, AIS analysis, live retrieval, or a sanctions determination, and it does not change the `decision_readiness_score`. Supply a dated `vessel_or_carrier_history` source to close it. The prevalence of these practices on Trans-Caspian and onward maritime legs is corroborated by recent maritime-detection research — dark ship-to-ship transfers ([arXiv:2404.07607](https://arxiv.org/abs/2404.07607)), dark AIS-off vessel trajectories ([arXiv:2502.01503](https://arxiv.org/abs/2502.01503)), and AIS / GNSS spoofing ([arXiv:2602.16257](https://arxiv.org/abs/2602.16257)) — which document why an AIS gap or an undisclosed STS transfer is a review trigger, not a benign data artifact. This is empirical context only: it is not the regulatory basis (OFAC maritime guidance is), and the worker flags the evidence gap rather than running any of these detection methods.
...
A counterparty may also declare two optional structural fields used for FFI sanctions-exposure flagging drawn from the OFAC FFI advisory (June 12, 2024) and FAQs 1148 / 1151: `specified_sectors[]` (an enum of `technology`, `defense_and_related_materiel`, `construction`, `aerospace`, `manufacturing`, `other`) and `date_of_formation` (ISO 8601 date). When `specified_sectors[]` includes a non-`other` value, the response surfaces "counterparty operates in an OFAC-named sector under EO 14024" in `top_risks` and a matching FFI sanctions-exposure entry in `exposure_layers.foreign_sanctions_exposure_layer`. When `date_of_formation` is on or after 2022-02-24 and the counterparty's jurisdiction matches a sanctioned, circumvention-watch, or named transshipment-hub country, the response surfaces "counterparty newly formed in a transshipment-risk jurisdiction" mirroring the OFAC FFI advisory red-flag example. Both are presence flags routed to human review; neither is a sanctions determination or an end-use ruling, and neither changes the `decision_readiness_score`.
