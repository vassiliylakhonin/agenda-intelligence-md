...
The example follows `schemas/v1/evidence-pack.schema.json`: it has a `topic`, an `evidence_mode`, optional `source_category`, claim-level `sources`, `unsupported_claims`, and a `source_plan`.
