...
toolkit gives you four checkpoints; the agent itself stays in your stack.

## Loop

```
1. event in              -->  agent receives a policy/risk update
2. source_plan(category) -->  agent fetches must_check + watch_indicators
3. agent drafts brief    -->  conforms to agenda-brief.schema.json
4. validate_brief        -->  schema check; agent fixes structural errors
5. validate_evidence     -->  evidence-pack schema check
6. (optional) audit-claims  evidence-audit.schema.json (claim-level)
7. score_output          -->  heuristic 0-100; gate or branch on threshold
8. emit                  -->  brief + evidence + audit JSON
```
...
`list_lenses`, `get_lens`, `source_plan`, `score_output`. See
[`MCP.md`](../../MCP.md) and [`mcp.md`](mcp.md) for client configs.

## What the agent owns

- Choosing `category` for `source_plan`.
